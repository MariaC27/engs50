...
int main(void){
	...
	hput(h1, (void*)d1, d1->name, strlen(d1->name));
	hput(h1, (void*)d2, d2->name, strlen(d2->name));
	hput(h1, (void*)d3, d3->name, strlen(d3->name));
	hput(h1, (void*)d4, d4->name, strlen(d4->name));
	hput(h1, (void*)d5, d5->name, strlen(d5->name));
	hput(h1, (void*)d6, d6->name, strlen(d6->name));
	hput(h1, (void*)d7, d7->name, strlen(d7->name));
	hput(h1, (void*)d8, d8->name, strlen(d8->name));
	hput(h1, (void*)d9, d9->name, strlen(d9->name));
	hput(h1, (void*)d10, d10->name, strlen(d10->name));
	hput(h1, (void*)d11, d11->name, strlen(d11->name));

	
	// here is
	dog_t* remove_res = (dog_t *)hremove(h1, search_queue, "sparky", 6);
	if (remove_res == NULL){
